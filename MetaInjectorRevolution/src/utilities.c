...
void listPatchedFiles(int patchedFilesCount)
{
    ...
    MetaFileInfo* metaFileInfo = getMetaFileInfo(getLatestBackup());
    FileBlock* fileBlocks = fillFileBlocks(metaFileInfo);

    FileBlock* patchedFiles = (FileBlock*) malloc(patchedFilesCount * sizeof(FileBlock));
    ...
    FILE* metaFile = openFile("pad00000.meta", "rb");
    printf("Searching patched files...");
    for (i = 0; i < metaFileInfo->fileBlocksCount; i++)
    {
    ...
    {
        if (patchedFilesCount < 10)
        {
            printf("%d", i+1);
        }
        else if (patchedFilesCount < 100)
        {
            printf("%.2d", i+1);
        }
        else if (patchedFilesCount < 1000)
        {
            printf("%.3d", i+1);
        }
        else
