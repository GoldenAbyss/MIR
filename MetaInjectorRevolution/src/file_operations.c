...
void copyFile(char* srcFileName, char* cpyFileName)
{
    ...
    copyFileTo(srcFileName,currentPath,currentPath);
    rename(srcFileName,cpyFileName);
}


// Copies an entire file, byte per byte, from a folder, to another
int copyFileTo(char* srcFileName, char* src_folder,char* dest_folder)
{
    ...
    char* dest_path = concatenate(dest_folder,srcFileName);
...
int systemCopyAndRename(char* srcFileName,char* destFileName, char* src_folder,char* dest_folder)
{
    char* originalNameFullPath = concatenate(dest_folder,srcFileName);
    char* renamedNameFullPath = concatenate(dest_folder,destFileName);

    int renameSucceded = !rename(originalNameFullPath,renamedNameFullPath);

    free(originalNameFullPath);
    free(renamedNameFullPath);

    return (systemCopy(srcFileName,src_folder,dest_folder) && renameSucceded);
}

int systemCopy(char* srcFileName, char* src_folder,char* dest_folder)
{
    char* currentPath = getCurrentPath();
    char* src_complete_path = (char*) malloc(strlen(currentPath) + strlen(src_folder) + 1);

    int success = 0;
    //createPath(src_complete_path);
    ...
    char* command = (char*) malloc(strlen("copy /y \"%s%s\" \"%s%s\\ 1>NULL3") + strlen(src_complete_path) + strlen(srcFileName) + strlen(dest_folder) + strlen(srcFileName) + 1);

    charReplace(src_complete_path,'/','\\');
    charReplace(dest_folder,'/','\\');
    sprintf(command,"copy /y \"%s%s\" \"%s%s\" 1>NULL3",src_complete_path,srcFileName,dest_folder,srcFileName);
    system(command);


    FILE* outputFile = fopen("NULL3", "rb");
    if (outputFile == NULL)
    {
        sprintf(command,"copy /y \"%s%s\" \"%s%s\"",src_complete_path,srcFileName,dest_folder,srcFileName);
        system(command);
        printf("\n\nTo solve this problem. Try running this program as administrator.");
        printf("\nIf that doesn't work. Move your entire game folder to your desktop and try again.\n");
        PAUSE();
        success = 0;
    }
    else
    {
        ...
        {
            char* outputRead = (char*) malloc(fileSize + 1);

            fread(outputRead,fileSize,1,outputFile);
            fclose(outputFile);
            outputRead[fileSize] = '\0';

            char* dest_complete_path = concatenate(dest_folder,srcFileName);
            src_complete_path = concatenate(src_complete_path,srcFileName);

            success = fileExists(dest_complete_path) && (getFileSizeByName(dest_complete_path) == getFileSizeByName(src_complete_path));

            if (success)
            {
                printColor(outputRead,GREEN);
            }
            else
            {
                printColor("Error: The file was not copied. ",RED);
                printf("%s", outputRead);
            }
        }
    }
